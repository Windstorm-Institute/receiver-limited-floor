...
\wsirunhead{The Receiver-Limited Floor}
...
\toprule
System & $M$ & $\varepsilon$ & $R_M(\varepsilon)$ & Support bound $\log_2 M$ & Analytic correction ($\log_2 M - R_M$) \\
\midrule
...
The final column is the analytic correction term $\log_2 M - R_M(\varepsilon) = H_b(\varepsilon) + \varepsilon \log_2(M-1)$. Because the ``Support bound'' entries are the maximum-entropy ceiling $\log_2 M$ rather than independent throughput measurements, this column is fixed by $M$ and $\varepsilon$ alone and is not a comparison against measured data. The only independent (Tier~1) measurement available for these systems is the Miller--Nicely confusion-matrix mutual information for phonology, $I(X;Y) \sim 4.20$ bits, against which the floor $R_{31}(0.02) = 4.71$ overpredicts by $-0.51$ bits (see the Empirical Phonology Values subsection).

\subsection{Non-Uniform Distribution Robustness}

Under non-uniform amino acid frequencies, the rate-distortion floor shifts from 4.390 to approximately 4.175 bits---a 0.2-bit reduction driven by amino-acid usage bias (the KL divergence of empirical frequencies from uniform). The support-size ceiling $\log_2(21) = 4.392$ therefore stands about 0.2 bits above the non-uniform floor. We are explicit that this gap is a difference between two theoretical quantities---the uniform ceiling and the usage-weighted floor---not a comparison against an independent measurement of ribosome throughput; establishing a genuine excess-capacity margin would require a Tier-1 confusion-matrix measurement of ribosomal decoding, which is not yet available. The correction is nonetheless informative: it shows the floor is sensitive to realistic usage statistics at the $\sim$0.2-bit level, comparable to the discrimination margins implied by Hopfield kinetic proofreading~\cite{hopfield1974}.
...
Matched-encoder models (EN$\to$X), which receive English input matching the corpus, operate at BPB $\sim 4.19$. This BPB is bits per \emph{byte} of the English reference corpus---a compression measure of a mismatched translation model---whereas the ribosome floor of 4.39 bits is bits per serial decoding \emph{event}; the two use different denominators, so their numerical proximity is a coincidence rather than a unit-matched agreement, and it is reported only as an exploratory observation that both quantities fall in the same narrow bit range. (The flagship causal language models, the closer analog of a serial decoder, sit at BPB $\sim 0.94$.) Fully mismatched models (X$\to$Y) operate at BPB $\sim 5.52$---a penalty of $+1.33$ bits. The groups are significantly different (Kruskal--Wallis $H = 230.2$, $p \sim 0$), but vocabulary independence holds within each group. This finding was not part of the original experiment design and emerged during post-hoc analysis. It is reported as an exploratory finding, though the effect size (1.33 bits) is large and consistent across 1,461 models.
...
\section*{Appendix A: Experiment Protocol}

The complete experiment protocol---hardware specifications, corpus preparation, BPB computation for three model formats (HuggingFace, GGUF, MarianMT), statistical-analysis procedures, and visualization specifications---is provided in the supplementary file \texttt{APPENDIX-A-EXPERIMENT-PROTOCOL.md} in the companion repository~\cite{whitmer2026rlf_code}. The protocol was designed for full reproducibility: any researcher with access to HuggingFace models and a CUDA-capable GPU can replicate the 1{,}749-model sweep from the provided instructions.

\section*{Appendix B: Figure Descriptions}

Eight figures accompany this paper (submitted separately):
\begin{itemize}
    \item \textbf{Figure 1.} Rate-distortion curves for $M \in \{4, 7, 12, 21, 31, 44, 64\}$; the biological systems (stars) sit inside the 3--5.5-bit convergence band.
    \item \textbf{Figure 2.} Predicted versus observed throughput for the three systems, with residual annotations.
    \item \textbf{Figure 3.} BPB versus vocabulary size across 1{,}749 models in four groups; regression lines are flat in all groups.
    \item \textbf{Figure 4.} BPB distribution by group (boxplot), showing four distinct operating regimes.
    \item \textbf{Figure 5.} Language-match effect on throughput (mean BPB, 95\% confidence intervals). EN$\to$X lands at 4.19 bits---\emph{bits per byte of English}, in the same narrow band as, but not a unit-matched comparison to, the ribosome floor.
    \item \textbf{Figure 6.} BPB density by group (kernel density estimate), relating each group to the throughput basin.
    \item \textbf{Figure 7.} The information cost of substrate mismatch: $+1.33$ bits between matched and mismatched processing.
    \item \textbf{Figure 8.} Vocabulary-independence statistics by group (summary-table visualization).
\end{itemize}

\subsection*{Data, Code \& Resources}
\noindent Manuscript source and archived record: \url{https://github.com/Windstorm-Institute/receiver-limited-floor} (Zenodo: \href{https://doi.org/10.5281/zenodo.19322972}{10.5281/zenodo.19322972}). Experiments, datasets, and analysis code: \url{https://github.com/Windstorm-Labs/receiver-limited-floor}. Windstorm Institute: \url{https://windstorminstitute.org}. Windstorm Labs: \url{https://windstormlabs.com}.
...
\vspace{2em}
\noindent\textit{This is Paper 2 of The Windstorm Series (Papers 1--9) on universal constraints in serial information processing.}
